...
$userCfgSource = "C:\Temp\user.cfg"
$userCfgDest = "$env:APPDATA\Kerio\VPN Client\user.cfg"
...
if ($statusCode -eq 200) {
    ...
    # Копіюємо user.cfg
    if (-not (Test-Path "$env:APPDATA\Kerio\VPN Client")) {
        New-Item -ItemType Directory -Path "$env:APPDATA\Kerio\VPN Client" -Force | Out-Null
    }

    if (Test-Path $userCfgSource) {
        Copy-Item -Path $userCfgSource -Destination $userCfgDest -Force
        Write-Host "  [OK] user.cfg скопійовано" -ForegroundColor Green
    } else {
        Write-Host "  [FAIL] Файл $userCfgSource не знайдено" -ForegroundColor Red
    }
